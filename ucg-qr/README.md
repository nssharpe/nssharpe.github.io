...
- **Favicon.** The mark's two left spokes dissolving into a QR ring plus three
  modules, generated by `build_ucg_favicon.py` and inlined as a data URI. Spoke
  geometry comes from the toolkit mark SVG; the rest is deliberately coarse
  because the full six-spoke mark and a real finder pattern both turn to mush at
  16px. `python build_ucg_favicon.py --variants` writes a comparison sheet — judge
  it by rasterising at 16px and magnifying, not by eye at display size.
